...
本项目是 `agent/<project-id>/` 模板下的独立 Agent 教程，保留自己的 `docs/`、`examples/` 和 Node.js 依赖；它不会复制 CS336 的 Lecture 目录。仓库根目录的 `course.yaml` 只负责把它登记到专题地图，后续如果需要课程化，再按[统一项目结构](../../standards/project-structure.md)增加 catalog 元数据。`docs/.vitepress/cache/`、`docs/.vitepress/dist/` 和 `node_modules/` 都是本地构建目录，不提交 Git。
