    ...
    .map((item) => {
      ...
      for (const output of outputs) {
        ...
        const page = [
          ...
          `<CourseHeader eyebrow=${JSON.stringify(`${course.shortTitle ?? course.title} · Lecture ${item.order ?? ''}`)} title=${JSON.stringify(item.title)} description=${JSON.stringify(item.subtitle ?? '')} status=${JSON.stringify(itemStatusLabel(item))} :details=${vueProp(details)} :links=${vueProp(links)} />`,
      ...
      const lectureLinks = [...(course.official?.slice(0, 1) ?? []), ...(item.official ?? [])]
      const lecturePage = [
        frontmatter({
