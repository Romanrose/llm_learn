...
用法：
  ...
  course export <course-id> <lecture-id>
  ...
  course validate <course-id> <lecture-id> [--provider deepseek] [--published]
...
function validateTranscriptFiles({ englishPath, chinesePath, notePath, blogPath, allowMissingEnglish = false, allowTimestampAnomalies = false }) {
  const errors = []
  const warnings = []
  for (const path of [chinesePath, notePath, blogPath]) {
    if (!existsSync(path) || !readFileSync(path, 'utf8').trim()) errors.push(`文件缺失或为空：${relativeToRepo(path)}`)
  }
  if (!allowMissingEnglish && (!existsSync(englishPath) || !readFileSync(englishPath, 'utf8').trim())) {
    errors.push(`文件缺失或为空：${relativeToRepo(englishPath)}`)
  }
  if (errors.length) return { errors, warnings }
  const english = existsSync(englishPath) ? readFileSync(englishPath, 'utf8') : ''
  ...
  if (!english && allowMissingEnglish) warnings.push(`缺少英文逐字稿，无法进行中英时间戳对齐：${relativeToRepo(englishPath)}`)
  if (english && !englishTimestamps.length) errors.push(`英文逐字稿没有段落时间戳：${relativeToRepo(englishPath)}`)
  if (!chineseTimestamps.length) errors.push(`中文逐字稿没有段落时间戳：${relativeToRepo(chinesePath)}`)
  if (english && englishTimestamps.length !== chineseTimestamps.length) {
  ...
  if (chineseOrder) {
    const message = '中文翻译时间戳发生倒序'
    if (allowTimestampAnomalies) warnings.push(`${message}，历史稿保留正文顺序并要求回听复核`)
  ...
  if (english && chinese.length < english.length * 0.28) errors.push(`中文稿长度异常：${chinese.length} 字符 / 英文 ${english.length} 字符`)
...
function publishedPaths(course, item) {
  const lectureDirectory = lectureNotesDirectory(course, item)
  return {
    lectureDirectory,
    english: join(lectureDirectory, 'transcript.en.md'),
    chinese: join(lectureDirectory, 'transcript.zh-CN.md'),
    note: join(lectureDirectory, 'note.md'),
    blog: join(lectureDirectory, 'blog.md'),
    review: join(lectureDirectory, 'review.yaml'),
  }
}

function validateTranscriptPaths(course, item, provider, args) {
  if (!args.includes('--published')) return { mode: 'candidate', paths: candidatePaths(course, item, provider), allowMissingEnglish: false }
  const paths = publishedPaths(course, item)
  let historical = false
  const sourceManifest = item.preparation?.manifest ?? join(paths.lectureDirectory, 'sources.yaml')
  const manifestPath = resolve(repoRoot, sourceManifest)
  if (existsSync(manifestPath)) historical = parse(readFileSync(manifestPath, 'utf8')).sourceState === 'historical'
  return { mode: 'published', paths, allowMissingEnglish: historical, allowTimestampAnomalies: historical }
}

function validate(courseId, lectureId, args) {
  if (!lectureId || lectureId.startsWith('--')) throw new Error('用法：course validate <course-id> <lecture-id> [--provider deepseek] [--published]')
  const provider = optionValue(args, '--provider', 'deepseek')
  const { data: course } = loadCourse(courseId)
  const item = courseItem(course, lectureId)
  const validation = validateTranscriptPaths(course, item, provider, args)
  const { paths } = validation
  const result = validateTranscriptFiles({
    englishPath: paths.english,
    chinesePath: paths.chinese,
    notePath: paths.note,
    blogPath: paths.blog,
    allowMissingEnglish: validation.allowMissingEnglish,
    allowTimestampAnomalies: validation.allowTimestampAnomalies,
  })
  console.log(`校验：${courseId}/${lectureId} · ${validation.mode === 'published' ? 'published' : provider}`)
  ...
  console.log(validation.mode === 'published'
    ? '校验通过：正式稿结构正常，可以继续构建网站。'
    : '校验通过：可以进入人工审核或 promote。')
}
...
async function exportLatex(courseId, lectureId) {
  ...
  console.log(`LaTeX/PDF 导出完成：${courseId}/${lectureId}`)
}
...
function status(courseId) {
  ...
  const lectureCounts = (course.items ?? []).reduce((result, item) => {
    const state = item.generation?.state === 'reviewed' || item.status === 'published'
      ? 'published'
      : item.generation?.state === 'draft-ready'
        ? 'draft'
        : item.preparation?.state ?? item.status ?? 'configured'
    result[state] = (result[state] ?? 0) + 1
    return result
  }, {})
  console.log(`${course.shortTitle ?? course.title}: resource review = ${review.state}`)
  console.log({ resources: counts, lectures: lectureCounts })
}
...
async function main() {
  ...
  if (command === 'export') return exportLatex(courseId, args[0])
