...
> This directory is the upstream lecture-material snapshot inside `llm_learn`. The root repository's Course CLI and VitePress site are the canonical entry points for notes, transcripts, PDFs, and deployment. The upstream Trace Viewer commands below are only for running the executable lectures locally.
